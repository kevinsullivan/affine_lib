...
| base : Π space_id, fm space_id
| deriv : Π space_id, (prod (pt K) (vec K)) → fm space_id → fm space_id 
-- TODO: curry all of these args

/-
space id -> dim -> Type u
-/
inductive fm' : nat → nat → Type u
| base : Π space_id dim, fm' space_id dim
| deriv : Π space_id dim, (prod (pt_n K) (fin dim → vec_n K)) → fm' space_id dim → fm' space_id dim
...
structure spc {aff_space_id : nat} (f : fm K aff_space_id) : Type u       -- interesting specimen, here, btw

-- 

def mk_space {aff_space_id : nat} (f : fm K aff_space_id) :=
  @spc.mk K _ _ aff_space_id f 
